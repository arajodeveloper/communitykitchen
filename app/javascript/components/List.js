  ...
  render(){
    ...
    if(this.state.foods.length > 0) {
      content = this.state.foods.map((food, idx) => {
        return <BoxMarker key={idx} lat={food.latitude} lng={food.longitude} icon={this.allIcons[idx % 3]} />
      })
    } else {
...
//     console.log(this.state.greenIcon);
    

    // return (
    //   <Map center={positionGreenIcon} zoom={this.state.zoom}>
    //     <TileLayer
    //       attribution='&amp;copy <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
    //       url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
    //     />
